 ...
 * @param {string | undefined} target
 */
function isGitHubPullRequestUrl(target) {
...
async function fetchPullRequestDiff(pullRequest, onProgress) {
  ...
  try {
    ...
    let bytes = 0;
    while (true) {
      // oxlint-disable-next-line react-doctor/async-await-in-loop -- ReadableStreamDefaultReader.read() advances a single stream cursor, so reads must remain sequential.
